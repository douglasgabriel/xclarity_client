module XClarityClient
    class API
        ...
        Dir["#{File.dirname(__FILE__)}/api/endpoints/**/*.rb"].each { |f| require f }
        ...
        def define_endpoints_getters()
            endpoints.each do |endpoint|
                self.class.send(:define_method, endpoint) { EndpointsRepository.endpoint_handler(endpoint.to_sym).new }
            end
        end
    end
end